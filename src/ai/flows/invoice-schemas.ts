...
export const ExtractedProductSchema = z.object({
  ...
  sale_price: z.number().optional().describe('The extracted sale price (unit price if available).'), 
...
export const PromptOutputSchema = z.object({
    ...
    invoice_details: z.object({
        ...
        invoice_total_amount: z.number().optional().describe("The final total amount stated on the invoice document, usually including any taxes or VAT. Look for keywords like 'סהכ', 'Total', 'Grand Total', 'סהכ לתשלום'. Extract ONLY the numerical value."),
        invoice_date: z.string().optional().describe("The date appearing on the invoice document (e.g., 'YYYY-MM-DD' or 'DD/MM/YYYY')."), // Added
        payment_method: z.string().optional().describe("The method of payment indicated on the invoice (e.g., 'Cash', 'Credit Card', 'Bank Transfer', 'Check')."), // Added
...
export const ScanInvoiceOutputSchema = z.object({
  ...
  invoiceDate: z.string().optional().describe("The date appearing on the invoice document."), // Added
  paymentMethod: z.string().optional().describe("The method of payment indicated on the invoice."), // Added
