...
export const TaxInvoicePromptOutputSchema = z.object({
    supplierName: z.string().optional().describe("The supplier's name identified on the document."),
    invoiceNumber: z.string().optional().describe("The invoice number found on the document."),
    totalAmount: z.number().optional().describe("The final total amount stated on the invoice document, usually including any taxes or VAT. Look for keywords like 'סהכ', 'Total', 'Grand Total', 'סהכ לתשלום'. Extract ONLY the numerical value."),
    invoiceDate: z.string().optional().describe("The date appearing on the invoice document (e.g., 'YYYY-MM-DD' or 'DD/MM/YYYY')."),
    paymentMethod: z.string().optional().describe("The method of payment indicated on the invoice (e.g., 'Cash', 'Credit Card', 'Bank Transfer', 'Check')."),
});
export type TaxInvoicePromptOutput = z.infer<typeof TaxInvoicePromptOutputSchema>;
