...
export async function syncInventoryAction(
  ...
  console.log(
    `[syncInventoryAction] Starting manual sync for ${systemId} by user ${
      userId || "unknown"
    }...`
  );
  ...
  if (!userId) {
    console.error(
      "[syncInventoryAction] User not authenticated for sync operation."
    );
    return {
      success: false,
      message: "User not authenticated. Please log in to perform this action.",
    };
  }
  ...
  if (!config || Object.keys(config).length === 0) {
    return {
      success: false,
      message: `POS configuration for ${systemId} is missing or empty.`,
    };
  }
  ...
  try {
    ...
    switch (systemId) {
      case "caspit":
        console.log(
          `[syncInventoryAction] Calling syncCaspitProductsAction with config...`
        );
        productSyncResult = await syncCaspitProductsAction(config);
        break;
      case "hashavshevet": // Ensure Hashavshevet case is handled
        console.log(
          `[syncInventoryAction] Calling syncHashavshevetProductsAction with config...`
        );
      ...
      default:
        return {
          success: false,
          message: `Manual sync currently not supported for ${systemId}.`,
        };
    }

    console.log(
      `[syncInventoryAction] Raw product sync result for ${systemId}:`,
      productSyncResult
    );
    ...
    if (!productSyncResult.success) {
      console.error(
        `[syncInventoryAction] Failed to fetch products from ${systemId}: ${productSyncResult.message}`
      );
    } else {
      console.log(
        `[syncInventoryAction] Successfully fetched ${
          productSyncResult.itemsSynced ?? 0
        } products from ${systemId}.`
      );
    }
    ...
    console.error(
      `[syncInventoryAction] Error during sync execution for ${systemId}:`,
      error
    );
    return {
      success: false,
      message: `Sync action failed unexpectedly: ${
