...
export interface PaymentDueDateDialogProps {
  isOpen: boolean;
...
const PaymentDueDateDialog: React.FC<PaymentDueDateDialogProps> = ({
  ...
  const [selectedOption, setSelectedOption] =
    useState<DueDateOption>("immediate");
  ...
  useEffect(() => {
    ...
    if (isOpen) {
      console.log(
        "[PaymentDueDateDialog] Dialog opened. Resetting to default state (immediate, today)."
      );
  ...
  const handleConfirm = () => {
    ...
    if (invoiceDate) {
      if (invoiceDate instanceof Timestamp) {
        ...
        baseDateForCalc = new Date(); // Fallback if provided invoiceDate is invalid
      }
    } else {
      baseDateForCalc = new Date(); // Fallback if no invoiceDate is provided
    }
    baseDateForCalc = startOfDay(baseDateForCalc); // Ensure we use start of day for consistent EOM calcs
    ...
    switch (selectedOption) {
      case "immediate":
        ...
        break;
      case "net30":
        dueDate = addDays(endOfMonth(baseDateForCalc), 30);
        break;
      case "net60":
        dueDate = addDays(endOfMonth(baseDateForCalc), 60);
        break;
      case "net90":
        dueDate = addDays(endOfMonth(baseDateForCalc), 90);
        break;
      case "eom":
        dueDate = endOfMonth(baseDateForCalc);
        break;
      case "custom":
        dueDate = customDate;
        break;
    ...
    console.log(
      "[PaymentDueDateDialog] Confirming with option:",
      selectedOption,
      "resulting due date:",
      dueDate
    );
    onConfirm(dueDate, selectedOption);
    onOpenChange(false);
  };

  const handleDialogCancel = () => {
    console.log(
      "[PaymentDueDateDialog] Cancelled by user action (Skip or Close button)."
    );
    onCancel();
    onOpenChange(false);
  };
  ...
  return (
    <Sheet open={isOpen} onOpenChange={handleSheetOpenChange}>
      <SheetContent
        side="bottom"
        className="h-auto max-h-[80vh] flex flex-col p-0 rounded-t-lg"
      >
        ...
        <div className="flex-grow p-4 sm:p-6 space-y-4 overflow-y-auto">
          ...
          >
            ...
            <div>
              <div className="flex items-center space-x-2">
                <RadioGroupItem value="net90" id="due_net90" />
                <Label
                  htmlFor="due_net90"
                  className="font-medium cursor-pointer"
                >
                  {t("payment_due_date_option_net90")}
                </Label>
              </div>
            </div>
            <div>
        ...
        <SheetFooter className="p-4 sm:p-6 border-t flex flex-col sm:flex-row gap-2 shrink-0">
          ...
          >
            ...
            {t("payment_due_date_confirm_button")}
          </Button>
        </SheetFooter>
      </SheetContent>
    </Sheet>
