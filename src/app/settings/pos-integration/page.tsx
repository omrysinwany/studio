...
export default function PosIntegrationSettingsPage() {
  ...
  const loadInitialData = useCallback(async () => {
    ...
    try {
      ...
      const savedSettings = await getUserSettingsService(user.id);
      if (savedSettings) {
        setSelectedSystemId(savedSettings.posSystemId || "");
        setConfigValues(savedSettings.posConfig || {});
      }
    } catch (error) {
  ...
  const handleSystemChange = (systemId: string) => {
    ...
    if (user) {
      getUserSettingsService(user.id)
        .then((savedSettings) => {
          if (savedSettings && savedSettings.posSystemId === systemId) {
            setConfigValues(savedSettings.posConfig || {});
          } else {
            setConfigValues({});
          }
  ...
  const handleInputChange = (
    field: keyof PosConnectionConfig,
    value: string
  ) => {
  ...
  const handleSaveChanges = async () => {
    ...
    try {
      await saveUserSettingsService(
        {
          posSystemId: selectedSystemId,
          posConfig: configValues,
        },
        user.id
      );
      toast({
  ...
  const handleSyncInventoryNow = async () => {
    ...
    try {
      const inventoryResult = await syncInventoryAction(
        configValues,
        selectedSystemId,
        user.id
      );
      ...
      if (
        ...
        try {
          ...
          await finalizeSaveProductsService(
            inventoryResult.products,
            `POS_Inventory_Sync_${selectedSystemId}_${
  ...
  const handleSyncSalesNow = async () => {
    ...
    try {
      const salesResult = await syncSalesAction(
        configValues,
        selectedSystemId,
        user.id
      );
