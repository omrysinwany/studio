...
import { getUserSettingsService, saveUserSettingsService, type UserSettings } from '@/services/backend';
...
export default function NotificationSettingsPage() {
  ...
  const handleSaveSettings = async () => {
    ...
    const days = reminderDaysBefore !== undefined && reminderDaysBefore !== null && !isNaN(Number(reminderDaysBefore)) ? Number(reminderDaysBefore) : undefined;
      ...
      <Card className="shadow-md scale-fade-in">
        <CardHeader>
          <CardTitle className="text-xl sm:text-2xl font-semibold text-primary flex items-center">
            <Bell className="mr-2 h-5 sm:h-6 w-5 sm:w-6" /> {t('settings_notification_page_title')}
          </CardTitle>
          <CardDescription>{t('settings_notification_page_description')}</CardDescription>
        </CardHeader>
        <CardContent className="space-y-4">
          <div>
            <Label htmlFor="reminderDays">{t('settings_notification_reminder_days_label')}</Label>
            <Input
              id="reminderDays"
              type="number"
              value={reminderDaysBefore === undefined ? '' : String(reminderDaysBefore)}
              onChange={(e) => {
              ...
              className="mt-1"
            />
            <p className="text-xs text-muted-foreground mt-1">{t('settings_notification_reminder_days_note')}</p>
          </div>
          <div className="flex justify-end pt-2">
            <Button onClick={handleSaveSettings} disabled={isSaving}>
              {isSaving ? (
                ...
                <><Save className="mr-2 h-4 w-4" /> {t('settings_notification_save_button')}</>
              )}
            </Button>
          </div>
        </CardContent>
