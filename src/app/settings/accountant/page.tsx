      ...
      <Card className="shadow-md scale-fade-in">
        <CardHeader>
          <CardTitle className="text-xl sm:text-2xl font-semibold text-primary flex items-center">
            <Mail className="mr-2 h-5 sm:h-6 w-5 sm:w-6" /> {t('settings_accountant_page_title')}
          </CardTitle>
          <CardDescription>{t('settings_accountant_page_description')}</CardDescription>
        </CardHeader>
        <CardContent className="space-y-4">
          <div>
            <Label htmlFor="accountantName">{t('settings_accountant_name_label')}</Label>
            <Input
              id="accountantName"
          ...
          <div>
            <Label htmlFor="accountantEmail">{t('settings_accountant_email_label')} <span className="text-destructive">*</span></Label>
            <Input
              id="accountantEmail"
              type="email"
          ...
          <div>
            <Label htmlFor="accountantPhone">{t('settings_accountant_phone_label')}</Label>
            <Input
              id="accountantPhone"
              type="tel"
          ...
          <div className="flex justify-end pt-2">
            <Button onClick={handleSaveSettings} disabled={isSaving || !accountantEmail.trim()}>
              {isSaving ? (
                <><Loader2 className="mr-2 h-4 w-4 animate-spin" /> {t('saving_button')}...</>
              ) : (
                <><Save className="mr-2 h-4 w-4" /> {t('settings_accountant_save_button')}</>
              )}
            </Button>
          </div>
        </CardContent>
