...
import {
  ...
  AlertDialogAction,
  AlertDialogCancel,
  AlertDialogContent as AlertDialogContentComponent, // Aliased import
  AlertDialogDescription as AlertDialogDescriptionComponent, // Aliased import
  AlertDialogFooter as AlertDialogFooterComponent, // Aliased import
  AlertDialogHeader as AlertDialogHeaderComponent, // Aliased import
  AlertDialogTitle as AlertDialogTitleComponent, // Aliased import
  AlertDialogTrigger,
...
const isValidImageSrc = (src: string | undefined | null): src is string => {
  ...
  // Allow data URIs, http, https, and relative paths starting with / or blob URIs
...
export default function DocumentsPage() {
  ...
  // const [filterScanStatus, setFilterScanStatus] = useState<InvoiceHistoryItem['status'] | ''>(''); // Removed, only showing 'completed'
  ...
  }, [user, authLoading, router, fetchInvoicesTrigger]); // fetchUserData removed to avoid loop, using trigger instead
  ...
  const handleSaveInvoiceDetails = async () => {
    ...
    try {
        const updatedInvoiceData: Partial<InvoiceHistoryItem> = {
            ...
            // errorMessage: editedInvoiceData.errorMessage || null, // Should not be user-editable
            ...
            // Don't update URIs or rawScanResultJson from this simple edit form
        };
        ...
        if (refreshedInvoice) {
            // Update selectedInvoiceDetails with potentially transformed dates
              ...
              {isEditingDetails && selectedInvoiceDetails._displayContext !== 'image_only' ? (
                <div className="space-y-3">
                    ...
                    {selectedInvoiceDetails.status === 'error' && (<div><Label htmlFor="editErrorMessage">{t('invoice_details_error_message_label')}</Label><Textarea id="editErrorMessage" value={editedInvoiceData.errorMessage || ''} onChange={(e) => handleEditDetailsInputChange('errorMessage', e.target.value as string)} disabled={isSavingDetails}/></div>)}
                     {/* Payment Status can be changed via specific buttons now, so not directly editable here if it's a simple string update */}
                         ...
                         <div>
                            <Label>{t('invoice_payment_status_label')}</Label>
                                <div className="flex gap-2 mt-1">
                                    <Button variant={editedInvoiceData.paymentStatus === 'unpaid' ? 'destructive' : 'outline'} size="sm" onClick={() => handlePaymentStatusUpdate(selectedInvoiceDetails!.id, 'unpaid')} disabled={isSavingDetails}>{t('invoice_payment_status_unpaid')}</Button>
                                    <Button variant={editedInvoiceData.paymentStatus === 'pending_payment' ? 'secondary' : 'outline'} size="sm" onClick={() => handlePaymentStatusUpdate(selectedInvoiceDetails!.id, 'pending_payment')} disabled={isSavingDetails}>{t('invoice_payment_status_pending_payment')}</Button>
                ...
                <>
                 <div className="space-y-2"><h3 className="text-md font-semibold text-primary border-b pb-1">{t('invoice_details_document_section_title')}</h3>
                      ...
                      {selectedInvoiceDetails.documentType !== 'paymentReceipt' && (
                         <div className="flex items-center"><strong className="mr-1">{t('invoice_details_status_label')}:</strong> {renderPaymentStatusBadge(selectedInvoiceDetails.paymentStatus, selectedInvoiceDetails.paymentDueDate)}</div>
                      )}
                  </div>
