...
const formatDisplayValue = (value: number | undefined | null, fieldType: 'currency' | 'quantity', t: ProductsTableProps['t']): string => {
...
export function ProductsTable({
  ...
  if (products.length === 0 && !isEditingDeliveryNoteProducts) {
    return <p className="text-muted-foreground py-4">{t('edit_invoice_no_products_in_scan')}</p>;
  }
        ...
        <Table className="min-w-full sm:min-w-[600px]">
          <TableHeader>
            <TableRow>
              ...
              <TableHead className="text-right px-2 sm:px-4 py-2">{isEditingDeliveryNoteProducts ? t('edit_invoice_th_actions') : ''}</TableHead>
            </TableRow>
          </TableHeader>
          <TableBody>
              ...
              <TableRow key={product.id}>
                <TableCell className="px-2 sm:px-4 py-2 max-w-[100px] sm:max-w-xs truncate">
                  {isEditingDeliveryNoteProducts ? (
                    ...
                    <span title={product.catalogNumber || undefined}>{product.catalogNumber || 'N/A'}</span>
                  )}
                </TableCell>
                <TableCell className="px-2 sm:px-4 py-2 max-w-[150px] sm:max-w-md truncate">
                  {isEditingDeliveryNoteProducts ? (
                    ...
                    <span title={product.description || undefined}>{product.description || 'N/A'}</span>
                  )}
                </TableCell>
                <TableCell className="text-right px-2 sm:px-4 py-2">
                  {isEditingDeliveryNoteProducts ? (
                    <Input type="number" value={formatInputValue(product.quantity, 'quantity')} onChange={(e) => handleInputChange(product.id, 'quantity', e.target.value)} className="w-20 sm:w-24 text-right h-9" disabled={isSaving}/>
                  ) : (
                    <span>{formatDisplayValue(product.quantity, 'quantity', t)}</span>
                  )}
                </TableCell>
                <TableCell className="text-right px-2 sm:px-4 py-2">
                  {isEditingDeliveryNoteProducts ? (
                    <Input type="number" value={formatInputValue(product.unitPrice, 'currency')} onChange={(e) => handleInputChange(product.id, 'unitPrice', e.target.value)} className="w-24 sm:w-28 text-right h-9" disabled={isSaving}/>
                  ) : (
                    <span>{formatDisplayValue(product.unitPrice, 'currency', t)}</span>
                  )}
                </TableCell>
                <TableCell className="text-right px-2 sm:px-4 py-2">
                  {isEditingDeliveryNoteProducts ? (
                    <Input type="number" value={formatInputValue(product.lineTotal, 'currency')} onChange={(e) => handleInputChange(product.id, 'lineTotal', e.target.value)} className="w-24 sm:w-28 text-right h-9" disabled={isSaving}/>
                  ) : (
                    <span>{formatDisplayValue(product.lineTotal, 'currency', t)}</span>
                  )}
                </TableCell>
                <TableCell className="text-right px-2 sm:px-4 py-2">
                  {isEditingDeliveryNoteProducts && (
                    <Button variant="ghost" size="icon" onClick={() => onRemoveRow(product.id)} className="text-destructive hover:text-destructive/80 h-8 w-8" disabled={isSaving}>
