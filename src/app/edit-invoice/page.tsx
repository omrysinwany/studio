...
const formatInputValue = (value: number | undefined | null, fieldType: 'currency' | 'quantity' | 'stockLevel', t: (key: string) => string): string => {
...
const handlePriceConfirmationComplete = (resolvedProducts: Product[] | null) => {
 ...
 const handleNewProductDetailsComplete = (updatedNewProductsFromDialog: Product[] | null) => {
    ...
    const handleCancelEditTaxDetails = () => {
        ...
        //setIsViewMode(true); // Removed to keep main view/edit state separate
    };
   ...
   if (!user && !authLoading) {
    ...
    // Combined condition for no products or critical scan error for new scans
                      ...
                      <CardContent>
                           {documentType === 'deliveryNote' && (
                             <div className="mt-4 flex flex-col sm:flex-row justify-between items-stretch sm:items-center gap-3">
                                <Button variant="outline" onClick={() => { handleAddRow(); setIsEditingDeliveryNoteProducts(true); }} className="w-full sm:w-auto">
  ...
  return (
    <div className="container mx-auto p-4 md:p-8 space-y-6">
        <Card className="shadow-md scale-fade-in overflow-hidden">
            <CardHeader>
                <div className="flex flex-row items-center justify-between">
                    ...
                    {!isEditingTaxDetails && (
                        <Button variant="ghost" size="icon" onClick={() => setIsEditingTaxDetails(true)} className="h-8 w-8 text-muted-foreground hover:text-primary">
        ...
        {documentType === 'deliveryNote' && (
            <div className="mt-6">
                 <div className="flex flex-row items-center justify-between mb-2">
                    ...
                    {!isEditingDeliveryNoteProducts && products.length > 0 && (
                         <Button variant="ghost" size="icon" onClick={() => setIsEditingDeliveryNoteProducts(true)} className="h-8 w-8 text-muted-foreground hover:text-primary">
                             <Edit className="h-4 w-4" />
                             <span className="sr-only">{t('edit_button')}</span>
                         </Button>
                     )}
                ...
                {products.length > 0 ? (
                 ...
                 {isEditingDeliveryNoteProducts && (
                     <CardFooter className="flex justify-between items-center pt-4 mt-2 px-0 pb-0">
        ...
        <div className="mt-6 flex flex-col sm:flex-row items-stretch gap-3">
            ...
            {(!isEditingTaxDetails && !isEditingDeliveryNoteProducts) && (
                 <div className="flex-grow flex flex-col sm:flex-row sm:justify-end gap-3">
                    <Button
                        onClick={handleSaveChecks}
                        disabled={isSaving || (documentType === 'deliveryNote' && products.length === 0 && isNewScan) || !isSupplierConfirmed || (documentType !== 'invoice' && !selectedPaymentDueDate)}
      ...
      {showPaymentDueDateDialog && isSupplierConfirmed && (documentType === 'deliveryNote' || documentType === 'invoice') && isNewScan && (
        <PaymentDueDateDialog
          ...
          onCancel={() => {
            ...
            // After skipping, proceed to actual save
            proceedWithActualSave();
          }}
        />
      )}
